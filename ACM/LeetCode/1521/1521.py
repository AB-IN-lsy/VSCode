...
class Solution:
    def closestToTarget(self, arr: List[int], target: int) -> int:
        st = Std.SparseTable(arr, lambda x, y: x & y)
        ...
        for r in range(n):
            while l <= r and (tmp := st.query(l, r)) <= target:
                mn = Math.min(mn, target - tmp)
                l += 1
        r = 0
        for l in range(n):
            while l <= r and r < n and (tmp := st.query(l, r)) > target:
                mn = Math.min(mn, tmp - target)
                r += 1
        return mn
